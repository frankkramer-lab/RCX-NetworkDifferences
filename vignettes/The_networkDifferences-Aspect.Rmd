...
*The networkDifferences-aspect in a nutshell:* This document explains the NetworkDifferences aspect in detail but this section summarizes the main ideas: 
...
```{r library, echo=FALSE, message=FALSE, include = TRUE, eval=TRUE}
...
```
```{r mbnTRUE_nodeNetwork2, eval=FALSE}
...
```
```{r mbnFALSE_nodeNetwork2, eval=FALSE}
visualize(nodeNetwork)
```
...
```
```{r mbnTRUE_NodeNetworkRepr2, eval=FALSE}
visualize(nodeNetwork)
```
...
```
```{r mbnTRUE_edgeNetwork2, eval=FALSE}
...
```
```{r mbnFALSE_edgeNetwork2, eval=FALSE}
visualize(edgeNetwork)
```
...
```
```{r mbnTRUE_nodeEdgeNetwork2, eval=FALSE}
...
```
```{r mbnTRUE_nodeEdgeNetworkRepr2, eval=FALSE}
visualize(nodeEdgeNetwork)
```
...
```
```{r realLife_nodeNetwork2, eval=FALSE}
...
```{r realLife_nodeAttributesNetwork}
...
```
```{r realLife_nodeAttributesNetwork2, eval=FALSE}
